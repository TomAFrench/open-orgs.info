...
export async function setup(sdk: Context) {
  ...
  const createPortfolioLoader = (addresses: string[], vestingAddresses?: string[], nativeTokens: string[] = []) => async () => {
    const [portfolio, vestingPortfolio] = await Promise.all([
      sdk.plugins.getPlugin('zerion').getPortfolio(addresses, nativeTokens),
      vestingAddresses ? sdk.plugins.getPlugin('zerion').getPortfolio(vestingAddresses, nativeTokens) : [],
    ])
